...
int Key_GetEvent_Down()
{
    uint8_t key = Key_Scan();
    return (key == 1) ? 1 : 0;
}

uint8_t Key_GetEvent_Right() { return 0; };
...
int menu_command_callback(enum _menu_command command, ...)
{
    ...
    case OLED_SHUTDOWN: // 无参有返，熄屏
    {
        OLED_Clear();
        OLED_Update();
        BKP_WriteBackupRegister(BKP_DR2, 0xA5A5); // 进入睡眠前在bkp存入标志位，亮-》灭==a5a5   灭-》亮==a5a0
    ...
    case SHOW_STRING: // 参数:( int16_t x, int16_t y, char *str ); 返回: uint8_t 字符串长度;
    {
        ...
        /* 按需使用参数 */
        OLED_ShowString(show_x, show_y, show_string, OLED_8X16);
...
void MENU_Event_and_Action(MENU_HandleTypeDef *hMENU)
{
    ...
    static uint32_t key_repeat_timer = 0;
    static uint8_t last_wheel_direction = 0;

    // 增加超时时间
    // if (Tick >= 100) // 将超时时间从100增加到10000
    // {
    //     hMENU->isRun = 0;
    // }
    ...
    if (menu_command_callback(GET_EVENT_ENTER))
    {
		...
		while (menu_command_callback(GET_EVENT_ENTER) != 0)
                ; // 每次按键只移动一项，不连续进入
    ...
    {
        int wheel_event = menu_command_callback(GET_EVENT_WHEEL);

        if (wheel_event != 0)
        {
            // 修改为每次按键只移动一项，不连续滚动
            hMENU->Wheel_Event = -wheel_event;
            MENU_UpdateIndex(hMENU);
            hMENU->AnimationUpdateEvent = 1;
            Tick = 0; // 重置超时计时

            while (menu_command_callback(GET_EVENT_WHEEL) != 0)
                ; // 减少等待时间并降低延迟
        }
    }
}
...
void MENU_ShowBorder(MENU_HandleTypeDef *hMENU) // 显示边框
{
    ...
    {
        OLED_ShowNum(20, 100, Key_GetEvent_ENTER(), 2, OLED_6X8);
